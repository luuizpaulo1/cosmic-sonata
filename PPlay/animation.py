...
class Animation(gameimage.GameImage):
    ...
    def __init__(self, image_file, total_frames, loop=True):
        ...
        self.set_sequence(0, self.number_of_frames, self.loop)
    ...
    def set_sequence_time(self, initial_frame, final_frame, total_duration, loop=True):
        ...
        for x in range(initial_frame, final_frame):
            self.frame_duration_ms.append(total_duration)

    """Sets the time for all frames."""

    def set_total_duration(self, time_ms):
        time_frame = float(time_ms) / self.number_of_frames
        self.total_duration = time_frame * self.number_of_frames
        for x in range(0, self.number_of_frames):
            self.frame_duration_ms.append(time_frame)
    ...
    def update(self):
        if self.playing:
            ...
            if (
                time_ms - self.last_update > self.frame_duration_ms[self.curr_frame]
    ...
    def draw(self):
        if self.drawable:
            ...
            clip_rect = Rect(self.curr_frame * self.width, 0, self.width, self.height)
